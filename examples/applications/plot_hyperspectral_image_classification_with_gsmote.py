...
# Fit the model
def clf_grid (estimator, param_grid, X, y):
    global grid, prediction
    grid = GridSearchCV(estimator, param_grid,  scoring='f1_macro', cv=CV).fit(X, y)
    prediction = grid.best_estimator_.predict(X_test)
clf_grid(estimator=lr_pipeline, param_grid=param_grid, X=X_train, y=y_train)
print(format(classification_report(prediction, y_test)))
...
cm = confusion_matrix(prediction, y_test, labels = np.unique(y_test))
