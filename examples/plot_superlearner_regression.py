...
Simulated 1D regression data with Super Learner algorithm.

Four different simulations from reference [3] are presented. For 
each simulation a superlearner regressor is fitted to the data using 
the default base estimators and the true model, the simulated data and 
the superlearner fit are ploted.
"""
...
for curve_id, curve in enumerate(curves):
    ...
    plt.xlabel("X")
    plt.ylabel("Y")
plt.legend(("Simulated points", "True model", "Super Learner fit"), ncol=3, bbox_to_anchor=(0, -0.1), loc="upper center")
plt.show()
