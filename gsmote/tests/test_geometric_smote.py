...
def test_make_geometric_sample_hypersphere(center, surface_point):
    ...
    point = _make_geometric_sample(
        center, surface_point, 0.0, 0.0, RANDOM_STATE)
    rel_point = point - center
    rel_surface_point = surface_point - center
    np.testing.assert_array_less(
        0.0, norm(rel_surface_point) - norm(rel_point))
...
def test_make_geometric_sample_half_hypersphere(
...
@pytest.mark.parametrize(
    'center,surface_point,truncation_factor', [
        (center, surface_point, truncation_factor) for center, surface_point in zip(
            CENTERS, SURFACE_POINTS) for truncation_factor in TRUNCATION_FACTORS])
def test_make_geometric_sample_line_segment(
        center, surface_point, truncation_factor):
    ...
    np.testing.assert_array_less(
        0.0, norm(rel_surface_point) - norm(rel_point))
    dot_product = np.abs(
        dot_product) if truncation_factor == 0.0 else (-1) * dot_product
...
@pytest.mark.parametrize('selection_strategy, truncation_factor, deformation_factor',
                         [
                             (selection_strategy,
                              truncation_factor, deformation_factor)
...
def test_gsmote_fit_resample_binary(
    ...
    np.testing.assert_array_less(
        X[n_maj - 1] - radius, X_resampled[n_maj + n_min])
...
def test_gsmote_fit_resample_multiclass(
    ...
    X, y = make_classification(random_state=RND_SEED, n_samples=n_samples,
                               weights=weights, n_classes=3, n_informative=5)
